...
# Function to perform imputation on a single iteration
perform_imputation <- function(seed_value, data, pred_matrix, iterations, ...) {
  set.seed(seed_value)
  mice::mice(
    data,
    maxit = iterations,
    m = 1,  # Each parallel process creates one imputation
    printFlag = FALSE,
...
# Main function that sets up parallel processing
parallel_mice <- function(data, m = 10, maxit = 10, seed = 4218, ...) {
  # Create predictor matrix once
  predM <- make.predictorMatrix(data)
  
  # Switch off UID from MI algorithm
  predM[,"TreatmentID"] <- 0 # Switch off ID from predicting
  predM["TreatmentID",] <- 0
  
  # Set up parallel processing
  future::plan(future::multisession, workers = min(parallel::detectCores() - 1, m))
  
  # Generate different seeds for each imputation
  set.seed(seed)
  seeds <- sample.int(10000, m)
  
  # Perform imputations in parallel with proper RNG handling
  imp_list <- future.apply::future_lapply(1:m, function(i) {
    imp <- perform_imputation(seeds[i], data, predM, maxit, ...)
    # Extract the first completed dataset from each imputation
    mice::complete(imp, 1)
  }, future.seed = TRUE)
  
  # Create a proper mice object from the parallel results
  # First, run a minimal mice model to get the structure right
  init_imp <- mice(data, maxit = 0, m = m)
  
  # Replace the imputed datasets
  for (i in 1:m) {
    init_imp$imp <- lapply(names(init_imp$imp), function(vname) {
      if (length(init_imp$imp[[vname]]) > 0) {
        # For each imputed variable, extract values from our parallel results
        imputed_indices <- which(is.na(data[[vname]]))
        if (length(imputed_indices) > 0) {
          values <- sapply(imp_list, function(ds) ds[[vname]][imputed_indices])
          # If only one missing value, convert to matrix
          if (length(imputed_indices) == 1) {
            values <- matrix(values, nrow = 1)
          }
          return(values)
        }
      }
      return(init_imp$imp[[vname]])
    })
    names(init_imp$imp) <- names(init_imp$imp)
  }
  
  # Make sure other metadata is consistent
  init_imp$iteration <- maxit
  init_imp$seed <- seed
