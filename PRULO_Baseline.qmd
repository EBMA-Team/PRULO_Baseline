...
Complete case analysis of the QuickDASH total score and EQ5D Index were summarised using the *gtsummary* package (@tbl-proms). The Glenohumeral Instability cohort displayed significantly lower upper limb disability (QuickDASH) and higher self-rated general health (EQ5D) compared to the other cohorts.
...
The imputed datasets for QDASH and EQ5D5L were modeled with a linear model, in both unadjusted (no covariates) and adjusted forms (see RECORD 12.5). A significant main effect for diagnostic group (cohort) was observed for both scores. The adjusted models corrected for optimism with respect to coefficients.
...
```{epoxy}
For example, the coefficient for Glenohumeral Instability returned {gtsummary::inline_text(QDASHfitimpUnadj,variable = Cohort,level = "Glenohumeral Instability", pattern = "{estimate}")} compared to the adjusted {gtsummary::inline_text(QDASHfitimpAdj,variable = Cohort,level = "Glenohumeral Instability", pattern = "{estimate}")}. 
...
The results identified differences between the expanded diagnostic groups, with the Glenohumeral Instability group significantly different to other groups for the QuickDASH, but not the EQ5D. However, the pairwise comparisons revealed estimates that were below the MCID for all significant comparisons for the QuickDASH.
