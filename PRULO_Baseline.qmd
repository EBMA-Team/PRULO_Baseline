...
DistDistr <- Mastersheet2 |> dplyr::filter(StraightDist < 200) |> ggplot(aes(y = Cohort, x = StraightDist)) + 
...
This represents a {round((12.57 - 7.79)/12.57*100,2)}% reduction in the size of the effect. The adjustment for EQ5D was greater.
```
...
Post-hoc pairwise comparisons revealed significant differences between the Glenohumeral Instability group compared to the General and Rotator Cuff groups for both condition-specific and general health scales.
